...
const NotificationBell = (props) => {
    ...
    return (
        <div>
            ...
            {showNotifications && (
                <div className="bg-gray-900 h-[600px] w-[480px] fixed -translate-x-[440px] rounded-lg drop-shadow overflow-hidden">
                        ...
                        {flags.length > 0 ? (
                            flags.map((flag, index) => (
                                <div key={index} className="mb-2 bg-gray-800 rounded-lg p-2">
                                    <h1><span className='font-semibold'>Exam: </span>{flag.exam_name}</h1>
                                    <p><span className='font-semibold'>Question: </span>{flag.question_num}</p>
                                    <p><span className='font-semibold'>Student ID: </span>{flag.user_id}</p>
                                    <p><span className='font-semibold'>Issue: </span>{flag.flag}</p>
                                    <button className='flex rounded-lg mt-2 bg-white/10 p-1'>
